...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == 'hf':
            ...
            from verl.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode, vLLMRolloutMCTS
            ...
            if vllm_mode == 'customized':
                ...
                custom_strategy = self.config.rollout.get('rollout_custom_strategy', 'default')
                if custom_strategy == 'default':
                    rollout = vLLMRollout(model_path=local_path,
                                        ...
                                        trust_remote_code=trust_remote_code)
                elif custom_strategy == 'mcts':
                    rollout = vLLMRolloutMCTS(model_path=local_path,
                                        config=self.config.rollout,
                                        tokenizer=self.tokenizer,
                                        model_hf_config=self.actor_model_config,
                                        device_mesh=rollout_device_mesh,
                    ...
                    raise NotImplementedError("custom_strategy must be 'default' or 'mcts'")
            else:
    ...
    def init_model(self):
        from verl.workers.actor import DataParallelPPOActorSpOnly
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActorSpOnly(config=self.config.actor,
        ...
        if self._is_ref:
            ...
            self.ref_policy = DataParallelPPOActorSpOnly(config=self.config.ref, actor_module=self.ref_module_fsdp)
...
class RewardModelWorker(Worker):
    """
    Modified to use AutoModelForCausalLMWithValueHead instead of AutoModelForTokenClassification
    """
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            values = logits[:, :, 0]  # (batch_size, seq_len) 获得取0的概率作为评分
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        def insert_data(user_input: str, all_ref_responses: list[str], response_ids, attention_mask, idx: int):
            """准备一条数据"""
            reference_responses = [
                item.split('</think>')[-1]
                for item in all_ref_responses
            ]
            # if len(reference_responses) >= 3:
            #     reference_responses = reference_responses[:3]
            # else:
            #     raise ValueError(f"reference_responses length should be at least 3, but got {len(reference_responses)}")
            reference_responses = [item.split('<|im_end|>')[0] for item in reference_responses]
            reference_responses = [{
                "rank": i+1, 
                "content": item
            } for i, item in enumerate(reference_responses)]
            ...
            # 删去需要去掉的符号
            remove_char = self.config.get('remove_char', [])
            # print(f"remove_char: {remove_char}")
            for char in remove_char:
                response = response.replace(char, '')
                for ref in reference_responses:
                    ref['content'] = ref['content'].replace(char, '')
            
            prompt = rm_instruction_template.format(
        ...
        for i in range(data.batch.batch_size[0]):
            # extract raw prompt
            user_input = data.non_tensor_batch['extra_info'][i]['user_input']
            all_ref_responses = data.non_tensor_batch['extra_info'][i].get('reference', {}).get('responses', [[]])
            if not isinstance(all_ref_responses[0], list):
                all_ref_responses = [all_ref_responses]
            response_ids = data.batch['responses'][i]
            attention_mask = data.batch['attention_mask'][i]
            for ref_responses in all_ref_responses:
                insert_data(user_input, ref_responses, response_ids, attention_mask, i)
        ...
        return DataProto.from_dict(rm_inputs), data_id
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        if self._do_switch_chat_template:
            rm_data, data_id = self._switch_chat_template(data)
        else:
            ...
            data_id = [i for i in range(data.batch.batch_size[0])]

        # print("in compute_rm_score")
        # assert False
        ...
        with self.ulysses_sharding_manager:
            ...
            real_scores = torch.full((data.batch.batch_size[0],), float('inf'), device=scores.device, dtype=scores.dtype)
            for i, idx in enumerate(data_id):
                real_scores[idx] = min(real_scores[idx], scores[i])
            scores = real_scores
            # print(f"len = {data.batch.batch_size[0]}, rm scores: {scores}")
