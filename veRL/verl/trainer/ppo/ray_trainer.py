...
class RayPPOTrainer(object):
    ...
    def _validate_config(self):
        ...
        # 2. Check if the batch size is divisible by the number of GPUs
        #    (if not using dynamic batch size)
    ...
    def _create_dataloader(self):
        ...
        if "custom_cls" in self.config.data and self.config.data.custom_cls.get("path", None) is not None:
            ...
            train_dataset_args["output_file_path"] = "./updated_dataset/train_dataset.jsonl"
            val_dataset_args["output_file_path"] = "./updated_dataset/val_dataset.jsonl"

        self.train_dataset = dataset_cls(**train_dataset_args)
        ...
        self.val_dataset = dataset_cls(**val_dataset_args)
        val_batch_size = len(self.val_dataset)
        # number of GPUs total
        n_gpus = self.config.trainer.n_gpus_per_node * self.config.trainer.nnodes
        val_batch_size = val_batch_size - (val_batch_size % n_gpus)
    ...
    def _validate(self):
        ...
        # self.val_dataset.save_dataset_to_file()
        ...
        for test_data in self.val_dataloader:
            ...
            # allow all validation now, not just rule-based
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                print(f'batch.non_tensor_batch: {batch.non_tensor_batch.keys()}')
                ...
                with _timer('step', timing_raw):
                    ...
                    # logger.log(data=f'batch: {batch.batch.keys()}', step=self.global_steps)
                    # logger.log(data=f'non_tensor_batch: {batch.non_tensor_batch.keys()}', step=self.global_steps)
                    # logger.log(data=f"{batch.non_tensor_batch['raw_prompt_ids']}", step=self.global_steps)
                    # assert False
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.use_rm:
                            ...
                            print(f"======== rm_wg over in ray_trainer =========")

                        # we combine with rule-based rm
                        reward_extra_infos_dict: dict[str, list]
                        # try:
                        print("before reward manager")
                        reward_result = self.reward_fn(batch, return_dict=True, epoch=epoch)
            ...
            # self.train_dataset.save_dataset_to_file()
