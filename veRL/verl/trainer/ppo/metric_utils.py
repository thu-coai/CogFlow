...
def get_list_average(l: list) -> float:
    l = [i for i in l if i != -998244353]
    if len(l) == 0: return -1
    return sum(l) / len(l)

def get_list_std(l: list) -> float:
    l = [i for i in l if i != -998244353]
    if len(l) == 0: return 0
    return np.std(l).item()
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    customized_info = {}
    for key in [
        'score', 'correctness', 'format_check', 
        'rm_score', 'rm_score_valid', 
        'len_reason', 'len_response', 'len_score', 'len_reason_reflong', 'len_reason_refshort', 
        'entropy', 'diversity_score', 
    ]:
        if key in batch.non_tensor_batch:
            customized_info[f'train_info/{key}_ave'] = get_list_average(batch.non_tensor_batch[key])
            customized_info[f'train_info/{key}_std'] = get_list_std(batch.non_tensor_batch[key])
...
def process_validation_metrics(data_sources: list[str],
    ...
    print(f"len data_sources: {len(data_sources)}")
    # import json
    # with open("tmp/infos_dict.txt", "w") as f:
    #     f.write(str(infos_dict))
    # with open("tmp/infos_dict.json", "w") as f:
    #     json.dump(infos_dict, f)
        ...
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                if isinstance(var_vals, list):
                    var_vals = [v for v in var_vals if v >= -1e5]
                    if len(var_vals) == 0:
                        continue
                elif isinstance(var_vals, np.ndarray):
                    var_vals: np.ndarray = var_vals[var_vals >= -1e5]
                    if var_vals.shape[0] == 0:
                        continue
                else:
                    raise ValueError(f'Unexpected type {type(var_vals)}')
                metric = {}
                n_resps = len(var_vals)
                metric[f"mean@{n_resps}"] = np.mean(var_vals)
                metric[f"std@{n_resps}"] = np.std(var_vals)

                if data_source in ['cog_flow']:
                    ns = []
                    n = 2
                    while n < n_resps:
                        ns.append(n)
